...
| `mise run winui:test` | 34 项通过、0 失败、0 跳过；覆盖配置保真、文本控件规范化、冲突/文件占用、Steam 解析/沙盒边界、Runner 替换与版本保护 |
...
该命令会创建新的示例 Steam 库和用户目录。发布目录是 `target/winui/publish`，必须保留整个目录。普通运行 `SteamWrapper.Manager.exe` 使用真实 Windows 用户数据路径；自动化验证使用沙盒。

当前 Manager 直接锁定 `Microsoft.WindowsAppSDK.WinUI` 2.3.6 与 `Microsoft.WindowsAppSDK.InteractiveExperiences` 2.1.6，对应原 Windows App SDK 2.4.0 组件集。最终新目录为 **171.196 MiB（179,511,987 字节、457 文件）**，包含 .NET/WinUI/Runner，未压缩；这不是安装包下载体积。
