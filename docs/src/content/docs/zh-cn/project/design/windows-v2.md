...
首个实现任务聚焦 A→B 的完整配置与受控启动切片，并增量建立 Windows 构建/契约 CI，保留现有工作流；C 后才切换默认发布链。工具链未就绪、配置无法无损往返或 Runner 生命周期回归未解决时，不以精美界面作为迁移完成证据。工具链已通过 mise 开始落地，最新安装与验证记录见 [Windows 开发环境](/SteamWrapper/zh-cn/development/windows/)。
