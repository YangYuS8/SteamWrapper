...
NewEpisode 在用户处理 Windows 安全提示后，已从普通资源管理器启动至标题并正常退出；主菜单英文、顶部菜单日文、退出确认中文，未进入剧情，Steam → Runner 尚未测试。Episode 1 在 11:30 本机时间的直接启动尝试中停在未知发布者安全提示，等待用户处理，尚无游戏启动证据。昨日入口缺失、云冲突及四部启动错误是旧安装布局的历史证据，不能替代对新目录的当前结论。成就自然触发和汉化版存档的 Steam 云同步仍未验证。
...
| NewEpisode / `1890120` | 用户处理安全提示后到达标题并正常退出；主菜单英文、顶部菜单日文、退出确认中文，未进入剧情 | 未测试 | 仅直接启动至标题通过，不代表完整汉化覆盖 |
| Episode 1 / `976390` | 11:30 从所在文件夹双击 `nine_kokoiro.exe`，出现未知发布者安全提示，尚无游戏启动证据 | 未测试 | 等待用户处理该提示；不沿用昨日入口缺失或云冲突结论 |
...
Episode 2 原始空启动选项在测试后恢复，Steam 属性中的云保持开启；随后只读提取该 AppID 的 localconfig 启动选项，确认值为空。没有复制或输出账户配置的其他字段。Episode 3、Episode 4 和 NewEpisode 的标题测试从普通资源管理器进行，没有据此声称经过 Runner。

NewEpisode 的安全提示与恢复分为两段：10:54 的 SmartScreen 页面导致首次暂停；用户告知自行关闭 SmartScreen 后，恢复尝试在 11:17 出现另一种“打开文件－安全警告”，显示未知发布者、“运行／取消”和“打开此文件前总是询问”。用户随后自行处理该提示，11:20:38 观察到目标进程及汉化组标题窗口；11:22:17 按中文退出确认正常退出，未进入 NewGame。测试代理未操作这些安全提示或改变保护设置。第一只恢复观察器因停止请求与用户操作交错，在目标仍运行时结束；补充观察器重新观察同一进程身份并记录其退出，不能将第一只观察器结束当作游戏退出。11:30 的 Episode 1 提示是后续独立检查点，仍待用户处理。
...
| NewEpisode | 68 个全部未变，无新增或缺失 | 64 个全部未变，无缺失 | 恢复后的直接启动新增 `savedata` 下 7 个 | 恢复后直接启动结束，3 个原文件全部未变 |
