...
| Episode 1 / `976390` | 用户处理提示后，`nine_kokoiro.exe` 在标题前报产品 ID 检查启动错误；确认普通错误框后退出 | 未测试、未创建配置 | 当前直接启动失败，不归因于 Runner |
...
| Episode 1 | 68 个全部未变，无新增或缺失 | 79 个全部未变，无新增或缺失 | 无 | 直接失败后 3 个原文件全部未变 |

上表复用各部最新完成的完整摘要：Ep2 是 Steam 测试及后续文件夹对照后，Ep3/Ep4/NewEpisode 是各自恢复后的 Steam 会话后，Ep1 是直接启动失败后。原文件均不变、无缺失；四部汉化目录中新产生的 `savedata` 由游戏正常运行写入，后续会话只更新这些本轮新文件，未覆盖初始基线中的原文件。没有将这些新文件清除或反向写回官方目录。
...
恢复测试结束后的汇总独立保存在 `resumed-final-scope-summary.json`：官方 340 个、汉化 354 个，共 694 个原始文件 SHA-256 均不变，无缺失；新增仅为 Ep2/Ep3/Ep4/NewEpisode 各 7 个、合计 28 个本地 `savedata` 文件。12:07 最终只读核验确认五部 15 个外部 remote 原文件全部不变、无新增或缺失，原备份完整；四部启动选项为空，Ep1 无该字段。各目录摘要按其最后测试阶段采集，不是同一时刻的全盘原子快照，旧的 11:02 暂停汇总保持原样。

目前已有这四份汉化安装的实际 Steam 启动成功证据。它们不能推及所有汉化包，也没有证明自然成就触发、Steam Auto-Cloud 会同步独立目录中的汉化存档、Job 成员关系或“启动器先退出而真实游戏继续”的场景。进程采样可能漏掉短暂进程；安全提示的判断依据是实际 Windows 界面，不能只凭采样中没有进程下结论。
