...
[English](windows-manager-comparison.md) | 简体中文
...
原生资源索引、.NET、WinUI、picker 投影、Runner 和清单均存在；Runner 摘要匹配，三种 Storage.Pickers 激活注册仍指向随包 `Microsoft.WindowsAppRuntime.dll`。locked restore 和 Release self-contained publish 通过。最终产物另行通过了沙盒原生窗口、配置读取、picker 打开/取消、保存和稳定 Runner 就绪复核，见[预览验收](winui-preview-validation.zh-CN.md)。**没有重测上述 6 轮内存或启动指标。**
