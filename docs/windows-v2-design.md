...
3. 查看只读的 Steam 安装位置，另外选择实际运行文件夹及其中的汉化 exe 或 launcher；实际文件夹可位于 Steam 库外，扫描不得覆盖它。参数、工作目录和等待方式放在“高级设置”，现有值必须完整保留。官方安装、汉化副本、存档与成就的关系见 [目录分离说明](translated-games.md)。
