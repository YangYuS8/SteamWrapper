...
English | [简体中文](windows-manager-comparison.zh-CN.md)
...
These are service-implementation differences. The old [save service](../crates/manager-core/src/lib.rs) reconstructs Profile, [configuration serialization](../crates/core/src/config.rs) regenerates the file, and [Runner installation](../crates/manager-core/src/runner.rs) selects replacement by hash. Dioxus can fix these behaviors; they are not caused by WebView or Rust. C# safeguards come from the new implementation/tests, not from WinUI alone.

<a id="依赖精简前的资源测量"></a>

## Resource measurements before component reduction

The [comparison script](../scripts/windows/Measure-ManagerComparison.ps1) measured sequentially after both builds/functional tests completed and test windows exited. Both used the same isolated directory with 10 profiles, 10 Steam manifests, local covers, and the same release Runner; the real Steam library was untouched.
...
Native resource index, .NET, WinUI, picker projections, Runner, and manifest are present. Runner hashes match and all three Storage.Pickers activation registrations still point to the bundled `Microsoft.WindowsAppRuntime.dll`. Locked restore and Release self-contained publish passed. The final artifact separately passed sandbox native-window launch, configuration loading, picker open/cancel, saving, and stable Runner readiness; see [preview validation](winui-preview-validation.md). **The six-round memory/startup measurements above were not rerun.**
